...
local tputests = base.BaseTest {
  ...
  command: utils.scriptCommand(
    |||
      ...
      coverage run --source=pytorch_lightning -m pytest -v --capture=no tests/callbacks/test_xla_stats_monitor.py tests/lite/test_lite.py tests/utilities/test_cli.py
      test_exit_code=$?
      echo "\n||| END PYTEST LOGS |||\n"
      coverage xml
